    ...
    class CEnvFadePatch : public IPatchRegistrar {
        public:
            ...
            virtual void register_patches(CMPPatchPlugin& plugin) override {
                ...
                plugin.register_patch<CSeqPatch>(CEnvFade_InputFade + 0x33, new SEQ_HEX("74 6b"), new SEQ_HEX("90 90"));

                SAnchor CEnvFade_InputReverseFade = anchors::server::CEnvFade::InputReverseFade.get(plugin.server_module());
                plugin.register_patch<CSeqPatch>(CEnvFade_InputReverseFade + 0x109, new SEQ_HEX("0f 85 49 ff ff ff"), new SEQ_HEX("e9 85 49 ff ff ff"));
            }
    };
};
