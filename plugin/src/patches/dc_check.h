    ...
    class CDCCheckPatch : public IPatchRegistrar {
        public:
            ...
            virtual void register_patches(CMPPatchPlugin& plugin) {
                ...
                plugin.register_patch<CPatch>(CPortalMPGameRules_ClientDisconnected + 0x82, SEQ_HEX("8b 4c 24 0c"), SEQ_HEX("66 e9 9d 00")); //Jump over the check
                ...
                plugin.register_patch<CPatch>(CPortal_Player_PlayerTransitionCompleteThink + 0x41, SEQ_HEX("74 54"), SEQ_HEX("eb 54")); //Patch the if check to always fail
            }
    };
};
